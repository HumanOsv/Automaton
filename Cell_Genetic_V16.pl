...
my $option_2D = 1;
my $option_3D = 1;
# procentaje de atomos mutados
my $PCENTATOMMUTATED   = 0.4;
# New species (1/5)*N
my $New_Pobl_Mut      = (1/5);
my $New_Pobl_Cell     = (1/5);
# Restart file
my $file_restart      = "Fit_Cell.xyz";
# Number of maximum cycles in automatum 
my $max_numb_convergence = 8;
# Check similarity
...
#
sub report_convergence {
	my ($info_array) = @_;
	my @info         = @{$info_array};
	#
	my $file_tmp = "04Files_not_converge.txt";
	open (FILE, ">>$file_tmp") or die "Unable to open file: $file_tmp";
	print FILE "\n";
	print FILE "   _____          __                         __                 	\n";
	print FILE "  /  _  \\  __ ___/  |_  ____   _____ _____ _/  |_ __ __  _____    	 \n";
	print FILE " /  /_\\  \\|  |  \\   __\\/  _ \\ /     \\\\__  \\\\   __\\  |  \\/     \\   	 \n";
	print FILE "/    |    \\  |  /|  | (  <_> )  Y Y  \\/ __ \\|  | |  |  /  Y Y  \\  	 \n";
	print FILE "\\____|__  /____/ |__|  \\____/|__|_|  (____  /__| |____/|__|_|  /  	 \n";
	print FILE "        \\/                         \\/     \\/                 \\/   	 \n";
	print FILE "	 \n";
	print FILE "        Global optimization of cluster geometries\n\n";
	print FILE "                     TiznadoLab\n";
	print FILE "\n";
	my $datestring = localtime();
	print FILE "              $datestring\n\n";	
	print FILE "# # Geometry Optimization Convergence Problems # #\n";
	print FILE "= = = = = = = = = = = = = = = = = = = = = = = = = \n";
	print FILE "Structures N°: ";
	print FILE scalar (@info);
	print FILE "\n";
	foreach my $i (@info) {
		print FILE "$i\n";
	}
...
sub parallel_cpu_local {
	...
	if ( $option == 0 ) {
		foreach my $i (@tmp_arr) {
			(my $without_ext = $i) =~ s/\.[^.]+$//;
			print SLURMFILE "g09 $without_ext.com\n";
		}
	} else {
		print SLURMFILE "g09 $file_one.com\n";	
	}
	#
	close (SLURMFILE);
	#
...
###################################
# submit gaussian
sub submit_queue_g09 { 
	my ($arrayInputs,$number_ele,$header,$ncpus,$mem,$Charge,$Multiplicity,$queue,$exec_bin_g09) = @_;
	#
...
sub Opt_Cell_Automata {
	my ($Num_of_geometries_input,$Atoms,$discretized_search_space,$Num_of_atoms,$id_name) = @_;
	#
	my @cartesian_coords = ();
	my @filename         = ();
	#
...
sub print_logo {
	print "\n";
	print "   _____          __                         __                 	\n";
	print "  /  _  \\  __ ___/  |_  ____   _____ _____ _/  |_ __ __  _____    	 \n";
	print " /  /_\\  \\|  |  \\   __\\/  _ \\ /     \\\\__  \\\\   __\\  |  \\/     \\   	 \n";
	print "/    |    \\  |  /|  | (  <_> )  Y Y  \\/ __ \\|  | |  |  /  Y Y  \\  	 \n";
	print "\\____|__  /____/ |__|  \\____/|__|_|  (____  /__| |____/|__|_|  /  	 \n";
	print "        \\/                         \\/     \\/                 \\/   	 \n";
	print "	 \n";
	print "        Global optimization of cluster geometries\n\n";
	print "                     TiznadoLab\n";
	print "\n";
	my $datestring = localtime();
	print "              $datestring\n\n";
...
if (not defined $file_name) {
	print_logo ();
	die "\nAutomatum must be run with:\n\nUsage:\n\t perl Cell_Genetic.pl [configure-file]\n";
	exit;  
}
#
my @delete_rot;
...
my @array_keys            = ();
#
my $file_tmp = "03Report_Output.txt";
open (FILEREP, ">$file_tmp") or die "Unable to open XYZ file: $file_tmp";
#
print FILEREP "\n";
print FILEREP "   _____          __                         __                 	\n";
print FILEREP "  /  _  \\  __ ___/  |_  ____   _____ _____ _/  |_ __ __  _____    	 \n";
print FILEREP " /  /_\\  \\|  |  \\   __\\/  _ \\ /     \\\\__  \\\\   __\\  |  \\/     \\   	 \n";
print FILEREP "/    |    \\  |  /|  | (  <_> )  Y Y  \\/ __ \\|  | |  |  /  Y Y  \\  	 \n";
print FILEREP "\\____|__  /____/ |__|  \\____/|__|_|  (____  /__| |____/|__|_|  /  	 \n";
print FILEREP "        \\/                         \\/     \\/                 \\/   	 \n";
print FILEREP "	 \n";
print FILEREP "        Global optimization of cluster geometries\n\n";
...
while ( $convergence_count < 1 ) {
	...
	if ($count_cycle == 0) {
		if ( $Num_of_geometries_input > 0 ) {
			print FILEREP "MESSAGE Initial Species (3D)\n";			
			if ( $type_Cellular_automaton == 1 ) {
				# Generar poblacion Automata celular
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($Num_of_geometries_input,\@Atoms,
																	\@discretized_search_space_3D,$Num_of_atoms,"Cell3D");
			} else {			
		...
		for (my $iteration = 0 ; $iteration < $Num_of_geometries_input ; $iteration++) {	
			...
			print FILEREP "MESSAGE Input $Input_QM\n";
		...
		if ( $type_Cellular_automaton == 1 ) {
			...
			if ( $option_1D == 1) {	
				...
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($new_numb,\@Atoms,
																	\@discretized_search_space_1D,$Num_of_atoms,"Cell1D");
				...
				#
				print FILEREP "MESSAGE New species 1D\n";
			}
			if ( $option_2D == 1) {	
				$filebase = ();
				$string_atoms_coords = ();	
				...
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($new_numb,\@Atoms,
																	\@discretized_search_space_2D,$Num_of_atoms,"Cell2D");
				...
				#
				print FILEREP "MESSAGE New species 2D\n";
			}
			if ( $option_3D == 1) {	
				$filebase = ();
				$string_atoms_coords = ();			
				...
				#
				print FILEREP "MESSAGE New species 3D\n";
			}	
		} else {
			# Generar poblacion tipo kick
			# Generar poblacion 1D
			my $new_numb = int ($fix_numb_new_cell/2);			
			if ( $option_1D == 1) {	
				($filebase,$string_atoms_coords) = pob_type_kick ($ma_x,"2","2",\@Atoms,$new_numb,"kick1D");
				#
				for (my $i = 0 ; $i < scalar (@{$filebase}) ; $i++) {
					push (@new_name_base  ,$$filebase[$i]);
					push (@new_coords_base,$$string_atoms_coords[$i]);
			...
			if ( $option_2D == 1) {	
				...
				($filebase,$string_atoms_coords) = pob_type_kick ($ma_x,$ma_y,"2",\@Atoms,$new_numb,"Kick2D");
				#
				for (my $i = 0 ; $i < scalar (@{$filebase}) ; $i++) {
					push (@new_name_base  ,$$filebase[$i]);
					push (@new_coords_base,$$string_atoms_coords[$i]);
			...
			if ( $option_3D == 1) {	
				...
				($filebase,$string_atoms_coords) = pob_type_kick ($ma_x,$ma_y,$ma_z,\@Atoms,$new_numb,"Kick3D");
				#
				for (my $i = 0 ; $i < scalar (@{$filebase}) ; $i++) {
					push (@new_name_base  ,$$filebase[$i]);
					push (@new_coords_base,$$string_atoms_coords[$i]);
	...
	my @rep_NImag	  = ();
	if ( $option_software == 0) {
		print FILEREP "MESSAGE Send g09\n";
		my ($value_energy_sort,$value_coords_sort,
			$report_structure,$report_NImag,$file_opt) = submit_queue_g09 (\@arrayInputs,$num_atoms_xyz,$header,$ncpus,$mem,$Charge,$Multiplicity,$queue,$exec_bin_g09);
		print FILEREP "MESSAGE End g09\n";	
		@opt_name      = @{$file_opt};
	...
	print FILEREP "\n";
	print FILEREP "Pre-Energy =  $minimun_energy\n";
	print FILEREP "# Coords $name_sort[0]\n";	
	...
	my ($not_struc_dup,$energy_dup,$files_dup,$coords_sim,$energy_sim,$files_sim) =  
	info_duplicate_structures ($Num_of_geometries_output,$num_atoms_xyz,\@files_coords,\@files_energy,\@files_name,$nprocess, $threshold_duplicate);
	#
	# Duplicados
	...
	print FILEREP "MESSAGE Cycle = $count_cycle\n\n";
#	final_opt_coords (\@files_energy,\@files_coords,\@files_name,$num_atoms_xyz,$option_software,"03Restart_coords.xyz");
	if ( $Info_conver_cell{$minimun_energy} > $max_numb_convergence ) {
		...
		print FILEREP "\n\nNormal Termination Genetic Cellular Automata\n\n";
		print FILEREP "Final-Energy = $minimun_energy\n";
		print FILEREP "# Final Coords $name_sort[0]\n";
		print FILEREP "$coord_xyz_sort[0]";
		print FILEREP "\n";		
		$convergence_count = 2;
	}
...
# Gaussian
if ( $option_software == 0 ) {
	report_convergence (\@files_not_converge);
	report_frequencies (\@files_NImag);
}
#
#
#
