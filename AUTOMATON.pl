...
sub Opt_Cell_Automata {
	my ($Num_of_geometries_input,$Atoms,$discretized_search_space,$Num_of_atoms,$id_name,$switchAU) = @_;
	#
	my @cartesian_coords = ();
	my @filename         = ();
	#
	...
	if ($count_cycle == 1) {
		...
		if ( $type_Cellular_automaton == 1 ) {
			...
			if ( $option_1D == 1) {	
				...
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($new_numb,\@Atoms, \@discretized_search_space_1D,$Num_of_atoms,"Cell1D_$count_cycle",$operator_DeepAU);
			...
			if ( $option_2D == 1) {	
				...
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($new_numb,\@Atoms,\@discretized_search_space_2D,$Num_of_atoms,"Cell2D_$count_cycle",$operator_DeepAU);
			...
			if ( $option_3D == 1) {	
				...
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($new_numb,\@Atoms,\@discretized_search_space_3D,$Num_of_atoms,"Cell3D_$count_cycle",$operator_DeepAU);
