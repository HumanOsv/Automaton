...
sub Rules_Cell_Automata {
	...
	if ( scalar (@tmp_new_neighbors_atoms) == 0 ) {
		...
		do {
            ...
            #my $flag1 = 0;
            #my $flag2 = 1;
            #if ( ( $element eq 'H' ) && ( $last_element eq 'H' ) ) {
            #    $flag1 = 1;
            #}
            #
            #$flag2 = steric_impediment_atoms (\@arr,0);
            #if (  $flag1 == 0  &&  $flag2 == 0  ) {
            #print "$flag1    $flag2 \n";
            #    $boolean = 0;
            #}
...
sub Opt_Cell_Automata {
	...
	for (my $iteration = 0 ; $iteration < $Num_of_geometries_input ; $iteration++) {
        ...
        #
        # New Rules Automaton 
        my @shuffled_atoms           = New_Rules_Automaton (\@{$Atoms});
        # Old Rules Automaton 
...
if ( $type_Cellular_automaton == 1 ) {
	# Atomata Cubico
	if( $sphereType == -1) {
		@discretized_search_space_1D = @{Construct_Discrete_Search_Space_Cube ($Box_x,"1.2","1.2","0.3",$sphereType)};
		@discretized_search_space_2D = @{Construct_Discrete_Search_Space_Cube ($Box_x,$Box_y,"1.2","0.3",$sphereType)};
...
while ( $convergence_count < 1 ) {
	...
	if ($count_cycle == 1) {
		if ( $Num_of_geometries_input > 0 ) {
			...
			if ( $type_Cellular_automaton == 1 ) {
				...
				#				
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($Porc_1D,\@Atoms, \@discretized_search_space_1D,$Num_of_atoms,"Cell1D_$count_cycle");
                print FILEREP "        Species 1D = $Porc_1D\n";
				($filebase_2d,$string_atoms_coords_2d) = Opt_Cell_Automata ($Porc_2D,\@Atoms,\@discretized_search_space_2D,$Num_of_atoms,"Cell2D_$count_cycle");
                print FILEREP "        Species 2D = $Porc_2D\n";
				($filebase_3d,$string_atoms_coords_3d) = Opt_Cell_Automata ($Porc_3D,\@Atoms, \@discretized_search_space_3D,$Num_of_atoms,"Cell3D_$count_cycle");
		...
		if ( $type_Cellular_automaton == 1 ) {
			...
			if ( $option_1D == 1) {	
				...
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($new_numb,\@Atoms, \@discretized_search_space_1D,$Num_of_atoms,"Cell1D_$count_cycle");
			...
			if ( $option_2D == 1) {	
				...
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($new_numb,\@Atoms,
																	\@discretized_search_space_2D,$Num_of_atoms,"Cell2D_$count_cycle");
			...
			if ( $option_3D == 1) {	
				...
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($new_numb,\@Atoms,
																	\@discretized_search_space_3D,$Num_of_atoms,"Cell3D_$count_cycle");
	...
	if ( $option_software == 0) {
		...
		my ($value_energy_sort,$value_coords_sort,
			$report_structure,$report_NImag,$file_opt) = submit_queue_gaussian (\@arrayInputs,$num_atoms_xyz,$header,$ncpus,$mem,$Charge,$Multiplicity,$queue,$exec_bin_g09);
	
		#
		print FILEREP "MESSAGE End Gaussian\n";
		#
		my $sumPort = $Global_Port_Normal + $Global_Port_Error;
