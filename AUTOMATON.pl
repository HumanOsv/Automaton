...
while ( $convergence_count < 1 ) {
	...
	if ($count_cycle == 1) {
		if ( $Num_of_geometries_input > 0 ) {
			...
			if ( $type_Cellular_automaton == 1 ) {
				...
				($filebase,$string_atoms_coords) = Opt_Cell_Automata ($Porc_1D,\@Atoms,
																	\@discretized_search_space_1D,$Num_of_atoms,"Cell1D_$count_cycle");
				($filebase_2d,$string_atoms_coords_2d) = Opt_Cell_Automata ($Porc_2D,\@Atoms,
																	\@discretized_search_space_2D,$Num_of_atoms,"Cell2D_$count_cycle");
				($filebase_3d,$string_atoms_coords_3d) = Opt_Cell_Automata ($Porc_3D,\@Atoms,
	...
	if ( $option_software == 0) {
		...
		print FILEREP "        Error  Termination Number = $Global_Port_Error/$sumPort\n";
		print FILEREP "        Error  Termination Files: \n";
